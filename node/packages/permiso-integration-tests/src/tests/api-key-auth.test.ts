  ...
  describe('with API key enabled', () => {
    ...
    it('should reject requests without API key', async () => {
      ...
      try {
        ...
        expect(error.networkError.result.errors[0].message).to.equal('API key required but not provided');
        expect(error.networkError.result.errors[0].extensions.code).to.equal('UNAUTHENTICATED');
      }
    });
    ...
    it('should reject requests with invalid API key', async () => {
      ...
      try {
        ...
        expect(error.networkError).to.exist;
        expect(error.networkError.statusCode).to.equal(401);
        expect(error.networkError.result.errors[0].message).to.equal('Invalid API key');
        expect(error.networkError.result.errors[0].extensions.code).to.equal('UNAUTHENTICATED');
      } finally {
    ...
    it('should allow mutations with valid API key', async () => {
      ...
      expect(result.data?.createOrganization.name).to.equal('Auth Test Organization');
    });
