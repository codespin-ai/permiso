...
describe('Field Resolvers and Nested Queries', () => {
  ...
  describe('Organization Field Resolvers', () => {
    it('should resolve nested users with pagination and filtering', async () => {
      const query = gql`
        query GetOrganizationWithUsers($id: ID!, $userFilter: UserFilter, $userPagination: PaginationInput) {
          organization(id: $id) {
            id
            name
      ...
      expect(result.data?.organization?.users?.nodes[0].id).to.equal('john-doe');
      ...
      expect(paginatedResult.data?.organization?.users?.nodes).to.have.lengthOf(2);
      expect(paginatedResult.data?.organization?.users?.totalCount).to.equal(3);
      expect(paginatedResult.data?.organization?.users?.pageInfo?.hasNextPage).to.be.true;
    });
    ...
    it('should resolve nested roles with filtering', async () => {
      ...
      expect(allRolesResult.data?.organization?.roles?.nodes).to.have.lengthOf(3);
    ...
    it('should resolve nested resources with filtering', async () => {
      const query = gql`
        query GetOrganizationWithResources($id: ID!, $resourceFilter: ResourceFilter) {
          organization(id: $id) {
            id
            name
      ...
      const resourceIds = result.data?.organization?.resources?.nodes.map((r: any) => r.id);
      expect(resourceIds).to.include.members(['/api/users', '/api/users/*', '/api/posts/*']);
  ...
  describe('User Field Resolvers', () => {
    it('should resolve user organization', async () => {
      ...
      expect(result.data?.user?.organization?.name).to.equal('ACME Corporation');
    ...
    it('should resolve user roles', async () => {
      ...
      const adminRole = result.data?.user?.roles.find((r: any) => r.id === 'admin');
      expect(adminRole?.properties).to.have.lengthOf(1);
      expect(adminRole?.properties[0]).to.deep.include({ name: 'level', value: 'super' });
    });
    ...
    it('should resolve user permissions', async () => {
      ...
      expect(result.data?.user?.permissions[0].resource?.name).to.equal('Admin Panel');
    });

    it('should resolve user effective permissions with filtering', async () => {
      const query = gql`
        query GetUserEffectivePermissions($orgId: ID!, $userId: ID!, $resourceId: String, $action: String) {
          user(orgId: $orgId, userId: $userId) {
            id
      ...
      const userApiPermissions = filteredResult.data?.user?.effectivePermissions;
      ...
      const writePermissions = actionFilteredResult.data?.user?.effectivePermissions;
  ...
  describe('Role Field Resolvers', () => {
    it('should resolve role organization', async () => {
      ...
      expect(result.data?.role?.organization?.name).to.equal('ACME Corporation');
    });
    ...
    it('should resolve role permissions', async () => {
      ...
      const actions = permissions.map((p: any) => ({ resourceId: p.resourceId, action: p.action }));
  ...
  describe('Resource Field Resolvers', () => {
    it('should resolve resource organization', async () => {
      ...
      expect(result.data?.resource?.organization?.name).to.equal('ACME Corporation');
    });
    ...
    it('should resolve resource permissions', async () => {
      ...
      const rolePermissions = result.data?.resource?.permissions.filter((p: any) => p.__typename === 'RolePermission');
      expect(rolePermissions.length).to.be.at.least(3); // admin has write/delete, viewer has read
      
      const adminPermissions = rolePermissions.filter((p: any) => p.roleId === 'admin');
  ...
  describe('Permission Field Resolvers', () => {
    it('should resolve permission organization', async () => {
      ...
      expect(result.data?.userPermissions[0].organization?.id).to.equal('acme-corp');
      expect(result.data?.userPermissions[0].organization?.name).to.equal('ACME Corporation');
    });
  ...
  describe('Deep Nested Queries', () => {
    it('should handle deeply nested queries efficiently', async () => {
      ...
      expect(role?.permissions[0]?.resource?.organization?.id).to.equal('acme-corp');
  ...
  describe('Complex Query Scenarios', () => {
    ...
    it('should handle field aliases correctly', async () => {
      ...
      expect(result.data?.primaryUser?.userRoles[0]).to.have.property('roleName');
    ...
    it('should handle fragments correctly', async () => {
      ...
      expect(user).to.include.all.keys(['id', 'identityProvider', 'identityProviderUserId', 'roles']);
      
      if (user?.roles?.length > 0) {
        const role = user.roles[0];
        expect(role).to.include.all.keys(['id', 'name', 'description', 'properties']);
      }
      
      expect(result.data?.roles?.nodes).to.have.length.at.least(1);
      const roleNode = result.data?.roles?.nodes[0];
      expect(roleNode).to.include.all.keys(['id', 'name', 'description', 'properties', 'users']);
    });
  });
});