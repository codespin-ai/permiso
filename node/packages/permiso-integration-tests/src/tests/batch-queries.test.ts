...
describe('Batch Queries', () => {
  ...
  describe('organizationsByIds', () => {
    ...
    it('should return empty array for non-existent IDs', async () => {
      ...
      const result = await client.query(query, { ids: ['non-existent-1', 'non-existent-2'] });
    ...
    it('should handle mixed existing and non-existing IDs', async () => {
      ...
      const result = await client.query(query, { ids: ['org-exists', 'org-not-exists'] });
  ...
  describe('usersByIdentity', () => {
    ...
    it('should only return users matching both provider and ID', async () => {
      ...
      expect(result.data?.usersByIdentity[0].identityProvider).to.equal('google');
    });
  });
});